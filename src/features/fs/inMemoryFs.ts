...
import type { FileSystemProvider, FileSystemWatcher, FsEntry } from "./types";
          ...
          (events) =>
            watcher(
              events.map((e) => {
                ...
                return {
                  ...e,
                  path: combine(parts.slice(0, i).join("/"), e.path),
                };
              }),
            ),
          options,
      ...
      watcher([
        {
          type: "created",
          entry: currEntry,
          path: combine(path, currEntry.name),
        },
      ]);
      watcher([{ type: "ready" }]);
    }
